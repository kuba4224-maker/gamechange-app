...
function selectHintsForPrompt({
  ...
  // 1. Odbiorca — do promptu zawodnika nigdy nie idzie treść 'rodzic'.
  const poOdbiorcy = wejscie.filter(isPlayerAudience);
  ...
  const poAktywnosci = poOdbiorcy.filter((h) => h.active !== false);
  const nieaktywne = poOdbiorcy.length - poAktywnosci.length;
