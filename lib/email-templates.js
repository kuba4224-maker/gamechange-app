...
// ------------------------------------------------------------
// ------------------------------------------------------------
// RODZIC C4 08.08.2026 — WARSTWA MATERIAŁÓW DLA RODZICA (decyzje A3, A9, C3).
//
// Trzy czyste funkcje niżej (`selectParentHints`, `describeParentActivity`,
// `describeParentChange`) są WSPÓLNYM MÓZGIEM raportu rodzica: używa ich
// i ten szablon e-maila, i `raport-rodzica.html`. Dzięki temu e-mail
// i strona nie mogą powiedzieć rodzicowi dwóch różnych rzeczy o tych
// samych danych — a to była realna wada poprzedniej wersji (dwie kopie
// tekstu utrzymywane ręcznie).
//
// ⚠️ BRAMKA WIEKOWA — CZYTAJ, ZANIM COKOLWIEK TU ZMIENISZ.
//
// Decyzja A9 działa w DWIE PRZECIWNE STRONY i to jest jej sedno:
//   • ZAWODNIK poniżej 16 lat NIE dostaje podpowiedzi z dawkami
//     suplementacyjnymi (`min_age = 16`). Nigdy. To pilnuje pas A,
//     w zapytaniu karmiącym prompt AI: `min_age IS NULL OR p_age >= min_age`.
//   • RODZIC dostaje je ZAWSZE, niezależnie od wieku dziecka — bo to on
//     kupuje suplement i on odpowiada za dawkę.
//
// Dlatego `selectParentHints()` NIE PRZYJMUJE WIEKU. To nie jest
// przeoczenie — to zabezpieczenie. Nie da się pomylić progu w funkcji,
// która o wieku nic nie wie. Jeżeli kiedykolwiek ktoś doda tu parametr
// wieku, żeby „ujednolicić z zawodnikiem", zrobi z tego dokładnie ten
// błąd, przed którym ta konstrukcja chroni: dawki suplementów podane
// jedenastolatkowi albo zabrane rodzicowi, który za nie płaci.
//
// Filtr jest jeden i dotyczy WYŁĄCZNIE odbiorcy: 'rodzic' albo 'oba'.
// `zawodnik` nie wchodzi tu nigdy — to treść pisana do dziecka, nie do
// dorosłego, i rodzic nie ma powodu jej czytać w tym miejscu.
// ------------------------------------------------------------
const PARENT_HINT_RECIPIENTS = ['rodzic', 'oba'];
const PARENT_HINT_LIMIT_DO = 6;   // ile „do zrobienia" pokazujemy
const PARENT_HINT_LIMIT_WIEDZA = 4; // ile „warto wiedzieć"

function selectParentHints(rows) {
  // ⚠️ Sygnatura bez wieku — patrz komentarz wyżej. Nie dodawaj.
  const empty = {
    doZrobienia: [], wartoWiedziec: [], razem: 0, zDawkami: 0, pominietych: 0, dostepnych: 0,
  };
  ...
  // Kolejność: najpierw to, co napisane WPROST do rodzica (odbiorca='rodzic'
  // — czyli m.in. wszystkie dawki), potem treść wspólna. Powód jest
  // praktyczny, nie estetyczny: przy sortowaniu wyłącznie po `pozycja`
  // podpowiedzi z dawkami (pozycje 8–12 w segmencie) wypadały poza limit,
  // czyli rodzic NIE dostawał dokładnie tego, co decyzja A9 każe mu dać.
  const kolejnosc = (a, b) => {
  ...
  return {
    ...
    // Limit jest widoczny, nie cichy — rodzic zobaczy zdanie „pokazuję N z M".
    pominietych: kandydaci.length - wybrane.length,
    dostepnych: kandydaci.length,
  };
}

// Źródło pod podpowiedzią: „Regeneracja — System Gamechange, s. 5, 13".
// Bez źródła rodzic nie ma jak sprawdzić, skąd to się wzięło — a to jest
// jedyna rzecz, która odróżnia tę treść od porady z internetu.
...
// ------------------------------------------------------------
// RODZIC C4 08.08.2026 — TRZY UCZCIWE STANY PUSTKI (reguła R5).
//
// Stara wersja raportu miała realny defekt tej klasy: przy zerze wpisów
// rodzic dostawał „Sesji treningowych: 0" i nie miał jak odróżnić
// „dziecko nie trenuje" od „dziecko trenuje, ale nie zapisuje". To są
// dwie ZUPEŁNIE różne wiadomości dla rodzica i mylenie ich jest gorsze
// niż nieinformowanie go wcale.
//
// Ta aplikacja NIGDY nie wie, czy dziecko trenuje. Wie wyłącznie, czy
// dziecko coś zapisało. Trzy stany mówią to wprost:
//   'aktywny'    — są zapisy, liczby coś znaczą
//   'cisza'      — konto żyje (diagnoza / cel / dawne wpisy), ale od N dni
//                  cisza. Mówimy wprost: to jest brak ZAPISU, nie brak treningu
//   'niezaczete' — nie ma diagnozy, celu ani ani jednego wpisu. Raport nie
//                  mówi nic o treningu, mówi tylko, że appka nie ruszyła
// ------------------------------------------------------------
function describeParentActivity({ report, lastLogAt, now } = {}) {
  const r = report || {};
  ...
  let dniOdWpisu = null;
  if (lastLogAt) {
    const t = new Date(lastLogAt).getTime();
    const teraz = now ? new Date(now).getTime() : Date.now();
    if (!Number.isNaN(t)) dniOdWpisu = Math.max(0, Math.floor((teraz - t) / 86400000));
  }
...
//
// To jest odpowiedź na „powód, żeby wrócić" bez naciskania: rodzic ma
// wiedzieć, co się ruszyło, a nie dostawać ten sam obrazek co miesiąc.
// Porównanie idzie z MIGAWKĄ ostatniego wysłanego raportu
// (`parent_report_snapshots`), nie z rekonstrukcją — nie zgadujemy.
//
// Świadomie ZERO paywalla, zero „kup teraz". PILOT_HIDE_PURCHASE zostaje
// w mocy. Ta sekcja ma być prawdziwa niezależnie od tego, czy kiedykolwiek
// pojawi się płatność.
// ------------------------------------------------------------
function describeParentChange({ report, previousReport, previousReportAt } = {}) {
  const cur = report || {};
  if (!previousReport) {
    return {
  ...
  const porownaj = (etykieta, kluczem) => {
    ...
    if (a === b) lines.push(`${etykieta}: ${a} — tyle samo co poprzednio.`);
    else if (a > b) lines.push(`${etykieta}: ${a} — poprzednio ${b}.`);
    else lines.push(`${etykieta}: ${a} — poprzednio ${b}.`);
  };
  ...
  let note = '';
  if (previousReportAt) {
    const d = new Date(previousReportAt);
    if (!Number.isNaN(d.getTime())) {
      note = `Porównanie z raportem z ${d.toLocaleDateString('pl-PL')}.`;
    }
  }
...
// ------------------------------------------------------------
// RODZIC C4 08.08.2026 — sekcja „Co możesz zrobić w domu w tym tygodniu".
// Ta jedna sekcja jest całą odpowiedzią na zarzut z audytu wartości
// produktu („prywatność wzorowa, treść uboga"): rodzic dostaje konkretne
// rzeczy do zrobienia, powiązane z obszarem, nad którym pracuje jego
// dziecko, KAŻDA ze wskazanym źródłem (materiał + strona).
//
// Trzy uczciwe stany także tutaj (R5) — bo „brak podpowiedzi" ma trzy
// zupełnie różne przyczyny i mylenie ich to dokładnie ten „cichy brak",
// który blok 3 nazwał po imieniu:
//   • baza podpowiedzi jeszcze nie wgrana → mówimy to wprost,
//   • wgrana, ale ten obszar nie ma treści dla rodzica → mówimy to wprost,
//   • jest treść → pokazujemy ją, z jawną informacją, ilu wskazówek nie
//     pokazaliśmy (limit widoczny, nigdy cichy).
// ------------------------------------------------------------
function buildParentHintsHtml({ extras, report }) {
  const ex = extras || {};
  const naglowek = '<p style="font-size:13px;letter-spacing:0.08em;text-transform:uppercase;color:#9a9488;margin:26px 0 8px 0;">Co możesz zrobić w domu w tym tygodniu</p>';
  const akapit = (t) => `<p style="font-size:14px;line-height:1.6;color:#3a3830;margin:0;">${escapeHtml(t)}</p>`;

  if (!ex.hints_available) {
    return naglowek + akapit(
      'Biblioteka wskazówek dla rodzica nie jest jeszcze podłączona do tego raportu. '
      + 'Nie pokazuję nic zamiast pokazywać coś wymyślonego — treść pojawi się tu, gdy tylko zostanie wgrana.'
    );
  }

  const wybor = selectParentHints(ex.hints);
  const segId = ex.segment_id || (report && report.priority_goal && report.priority_goal.segment_id) || null;
  ...
  const pozycja = (h) => {
    ...
    return `<li style="margin-bottom:10px;">${escapeHtml(h.hint)}`
      + (zrodlo ? `<br><span style="font-size:12px;color:#9a9488;">${escapeHtml(zrodlo)}</span>` : '')
      + '</li>';
  };

  let html = naglowek + akapit(wstep);
  if (wybor.doZrobienia.length) {
    html += `<ul style="margin:12px 0 0 0;padding-left:18px;font-size:14px;line-height:1.6;color:#3a3830;">${wybor.doZrobienia.map(pozycja).join('')}</ul>`;
  }
  if (wybor.wartoWiedziec.length) {
    html += '<p style="font-size:13px;letter-spacing:0.08em;text-transform:uppercase;color:#9a9488;margin:18px 0 8px 0;">Warto, żebyś to wiedział</p>';
    html += `<ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.6;color:#3a3830;">${wybor.wartoWiedziec.map(pozycja).join('')}</ul>`;
  }
  if (wybor.zDawkami > 0) {
    // Decyzja A9, strona rodzica. Zdanie jest tu po to, żeby rodzic
    // wiedział, że dziecko TEGO NIE WIDZI — i dlaczego.
    html += '<p style="font-size:13px;line-height:1.6;color:#3a3830;background:#f5f2ec;padding:14px 16px;border-left:3px solid #E8432D;margin-top:16px;">'
      + 'Wskazówki z konkretnymi dawkami widzisz Ty, nie dziecko. Aplikacja nie podaje dawek suplementów zawodnikom poniżej 16 lat — o tym decyduje dorosły, który kupuje i odpowiada. '
      + 'Jeśli cokolwiek tu budzi Twoją wątpliwość, to jest rozmowa z lekarzem albo dietetykiem, nie z aplikacją.</p>';
  }
  if (wybor.pominietych > 0) {
    html += `<p style="font-size:12px;color:#9a9488;margin-top:10px;">Pokazuję ${wybor.razem} z ${wybor.dostepnych} wskazówek, żeby ten e-mail dało się przeczytać na raz. Reszta wróci w kolejnych raportach.</p>`;
  }
  return html;
}

function buildParentHintsText({ extras, report }) {
  const ex = extras || {};
  if (!ex.hints_available) {
  ...
  const segNazwa = segId ? (SEG_NAMES[segId] || segId) : null;
  if (wybor.razem === 0) {
    return `\nCO MOŻESZ ZROBIĆ W DOMU\n${segNazwa ? `Dla obszaru „${segNazwa}” nie mamy jeszcze wskazówek dla rodzica.` : 'Nie mamy jeszcze wskazówek dopasowanych do pracy dziecka.'}\n`;
  }
  const linia = (h) => {
    const z = parentHintSource(h);
    return `- ${h.hint}${z ? `\n  (${z})` : ''}`;
  };
  let t = '\nCO MOŻESZ ZROBIĆ W DOMU W TYM TYGODNIU\n';
  if (segNazwa) t += `Dziecko pracuje teraz nad obszarem: ${segNazwa}.\n`;
  if (wybor.doZrobienia.length) t += wybor.doZrobienia.map(linia).join('\n') + '\n';
  if (wybor.wartoWiedziec.length) t += '\nWARTO, ŻEBYŚ TO WIEDZIAŁ\n' + wybor.wartoWiedziec.map(linia).join('\n') + '\n';
  if (wybor.zDawkami > 0) {
    t += '\nWskazówki z konkretnymi dawkami widzisz Ty, nie dziecko. Aplikacja nie podaje dawek suplementów zawodnikom poniżej 16 lat.\n';
  }
  if (wybor.pominietych > 0) {
    t += `\nPokazuję ${wybor.razem} z ${wybor.dostepnych} wskazówek. Reszta wróci w kolejnych raportach.\n`;
  }
  return t;
}

function parentReportEmail({ report, unsubscribeUrl, extras }) {
  ...
  // ---- RODZIC C4 08.08.2026 — trzy nowe bloki: stan, zmiana, materiały ----
  const ex = extras || {};
  const aktywnosc = describeParentActivity({ report, lastLogAt: ex.last_log_at });
  const zmiana = describeParentChange({
    report,
    previousReport: ex.previous_report,
    previousReportAt: ex.previous_report_at,
  });

  const stanHtml = aktywnosc.state === 'aktywny'
    ? ''
    : `<p style="font-size:14px;line-height:1.6;color:#0e0d0b;background:#f5f2ec;padding:14px 16px;border-left:3px solid #E8432D;margin:0 0 18px 0;"><strong>${escapeHtml(aktywnosc.headline)}</strong><br>${escapeHtml(aktywnosc.body)}</p>`;

  const zmianaHtml = `
    <p style="font-size:13px;letter-spacing:0.08em;text-transform:uppercase;color:#9a9488;margin:24px 0 8px 0;">Co się zmieniło od ostatniego raportu</p>
    ${zmiana.hasPrevious
      ? `<ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.7;color:#3a3830;">${zmiana.lines.map((l) => `<li>${escapeHtml(l)}</li>`).join('')}</ul>`
        + (zmiana.note ? `<p style="font-size:12px;color:#9a9488;margin-top:6px;">${escapeHtml(zmiana.note)}</p>` : '')
      : `<p style="font-size:14px;line-height:1.6;color:#3a3830;margin:0;">${escapeHtml(zmiana.note)}</p>`}
  `;

  const podpowiedziHtml = buildParentHintsHtml({ extras: ex, report });
  ...
  // RODZIC C4 08.08.2026 — te same trzy bloki w wersji tekstowej.
  const stanText = aktywnosc.state === 'aktywny'
    ? ''
    : `\n${aktywnosc.headline.toUpperCase()}\n${aktywnosc.body}\n`;
  const zmianaText = '\nCO SIĘ ZMIENIŁO OD OSTATNIEGO RAPORTU\n'
    + (zmiana.hasPrevious
      ? zmiana.lines.map((l) => `- ${l}`).join('\n') + '\n' + (zmiana.note ? `${zmiana.note}\n` : '')
      : `${zmiana.note}\n`);
  const podpowiedziText = buildParentHintsText({ extras: ex, report });
...
function retentionReminderEmail({ playerName } = {}) {
  ...
  const subject = 'Wszystko w porządku? Od kilku dni cisza w Twoim Dzienniku';

  const bodyHtml = `
    <p style="font-size:15px;line-height:1.6;color:#3a3830;">${greeting}, zauważyliśmy, że od kilku dni nie uzupełniasz swoich danych w aplikacji — wszystko w porządku?</p>
    <p style="font-size:14px;line-height:1.6;color:#3a3830;">Regularne wpisy w Dzienniku to podstawa, dzięki której system może przygotować dla Ciebie najlepsze możliwe rekomendacje i pomóc Ci trenować maksymalnie efektywnie. Wróć do uzupełniania — zajmie dosłownie chwilę.</p>
  ...
  return {
    subject,
    html: wrapHtml({ title: 'Wszystko w porządku?', bodyHtml }),
    text:
      `${greeting}, zauważyliśmy, że od kilku dni nie uzupełniasz swoich danych w aplikacji — wszystko w porządku?\n\n` +
      `Regularne wpisy w Dzienniku to podstawa, dzięki której system może przygotować dla Ciebie najlepsze możliwe rekomendacje i pomóc Ci trenować maksymalnie efektywnie.\n\n` +
      `Otwórz: ${APP_URL}\n`,
  };
}
...
//     Świadomie lekka treść (ma się czytać w ~20 sekund).
//
//     REDESIGN 06.08.2026 (zatwierdzony przez Kubę, claude/BRIEF_DELEGACJA_
//     PULS_TYGODNIOWY.md) — dopisane DWIE liczby ROZWOJOWE pod istniejącymi
//     dwiema liczbami UŻYCIA appki (te zostają bez zmian, mają wartość jako
//     szybki kontekst): cele osiągnięte w tym tygodniu + zbiorcza liczba
//     aktywnych sygnałów ryzyka/przeciążenia w tym tygodniu (jedna suma —
//     team_overload + player_risk_standout, decyzja redakcyjna z briefu).
//     Poprzedni komentarz w tym miejscu mówił, że team_overload świadomie
//     NIE jest tu powtarzany, bo dubluje Digest #19 — to wciąż prawda dla
//     TREŚCI Digestu (konkretny cytat/e-mail per sygnał), ale NIE dla samej
//     LICZBY: jeden zbiorczy licznik w cotygodniowym podsumowaniu nie
//     duplikuje zdarzeniowych e-maili Digestu, tylko daje ogólny obraz.
function weeklyTeamPulseEmail({
  teamName, rosterCount, activePlayersCount, activeFocusBlocksCount,
  goalsAchievedCount, riskSignalsCount,
} = {}) {
  const team = teamName || 'Twoja drużyna';
  const roster = rosterCount || 0;
  const active = activePlayersCount || 0;
  const blocks = activeFocusBlocksCount || 0;
  const goals = goalsAchievedCount || 0;
  const risk = riskSignalsCount || 0;
  ...
  const bodyHtml = `
    ...
    <p style="font-size:15px;line-height:1.6;color:#0e0d0b;background:#f5f2ec;padding:14px 18px;border-left:3px solid #E8432D;margin-top:12px;">
      <strong>${escapeHtml(goals)}</strong> ${goals === 1 ? 'osiągnięty cel' : 'osiągniętych celów'} w drużynie w tym tygodniu.<br>
      <strong>${escapeHtml(risk)}</strong> ${risk === 1 ? 'aktywny sygnał ryzyka/przeciążenia' : 'aktywnych sygnałów ryzyka/przeciążenia'} w tym tygodniu.
    </p>
    <p style="margin-top:20px;"><a href="${COACH_APP_URL}" style="display:inline-block;padding:12px 20px;background:#0e0d0b;color:#f5f2ec;text-decoration:none;font-size:14px;">Otwórz panel trenera</a></p>
  `;
  ...
  return {
    ...
    text: `Drużyna: ${team}\n\n` +
      ...
      `${goals} ${goals === 1 ? 'osiągnięty cel' : 'osiągniętych celów'} w drużynie w tym tygodniu.\n` +
      `${risk} ${risk === 1 ? 'aktywny sygnał ryzyka/przeciążenia' : 'aktywnych sygnałów ryzyka/przeciążenia'} w tym tygodniu.\n\n` +
      `Otwórz panel trenera: ${COACH_APP_URL}\n`,
  };
}
...
module.exports = {
  ...
  // RODZIC C4 08.08.2026 — czyste funkcje warstwy rodzica, wystawione
  // osobno, bo są testowane wprost (tests/test-email-templates.js) i bo
  // `raport-rodzica.html` powtarza dokładnie tę samą logikę u siebie.
  // ⚠️ selectParentHints NIE PRZYJMUJE WIEKU — patrz komentarz przy definicji.
  selectParentHints, describeParentActivity, describeParentChange, parentHintSource,
};
